...
| `vega search <关键词>` | 可读列表 | 搜索条目，逗号分隔多关键词，广泛召回（匹配标题、标签、描述、路径，不搜正文） |
| `vega search --project <项目名>` | 可读列表 | 模糊搜索项目，不确定项目名时使用 |
| `vega read <路径>` | md 原文 | 读取完整条目（含 frontmatter 和正文） |
| `vega write <路径>` | JSON | 创建新条目，description 和 tags 必填，正文从 stdin 读取 |
| `vega edit <路径>` | JSON | 编辑已有条目，stdin 读取 JSON 格式的替换内容 |
| `vega delete <路径>` | JSON | 删除条目 |
| `vega check` | 可读文本 | 知识库自检（格式、键统计） |
| `vega list [路径前缀]` | 可读列表 | 列出指定目录下的条目 |
...
- edit 从 stdin 读取 JSON 格式的替换内容（old/new/replace_all 字段），避开 shell 参数转义问题
