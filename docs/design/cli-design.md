...
|---|---|---|---|
| `vega init` | JSON | data | — |
| `vega search` | 可读列表 | query | limit（默认 50）、project（默认 false） |
...
- 写操作（init/write/edit/delete/rebuild）输出 JSON，read 输出 md 原文，search、list 和 check 输出可读文本
...
- search `project: true` 提供项目级搜索，匹配 _index.md 中的 name/remote/description
